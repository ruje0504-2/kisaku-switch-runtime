...
${CC:-cc} -std=c11 -O2 -Wall -Wextra -Werror -Iruntime $(pkg-config --cflags sdl2) \
    ...
    -lSDL2_test $(pkg-config --libs sdl2) $(pkg-config --cflags --libs libavformat libavcodec libswscale libswresample libavutil freetype2) -I"$mesa/include" -L"$mesa/lib" -lGLESv2 -o build/present-sdl-test
