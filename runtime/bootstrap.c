...
const char *bootstrap_ui_text(const KBootstrap *b,const char *key){
    const char *value=kui_translation_get(b?&b->ui_translation:NULL,key);
    return value?value:kui_translation_default(key);
}
...
KFont *bootstrap_ui_font(KBootstrap *b){
    ...
    int simplified=b->ui_translation.count||text_encoding(b)==KTEXT_GBK;
...
KBootstrap *bootstrap_create_split(const char *root,const char *save_root){
    ...
    snprintf(path,sizeof(path),"%s/zh_CN.txt",root);
    if(ktranslation_load(&b->translation,path)){error(b,"translation table invalid");return b;}
    snprintf(path,sizeof(path),"%s/ui_zh_CN.txt",root);
    if(kui_translation_load(&b->ui_translation,path)){error(b,"UI translation table invalid");return b;}
...
static int choice_page_text(KBootstrap *b,unsigned page,unsigned count,int top){
    ...
    for(unsigned i=0;i<count;i++){
        ...
        int width=0;for(size_t j=0;j<n;j++)width+=(int)chars[j].columns*8;
        int x=(640-width)/2;
...
int bootstrap_dispatch(KBootstrap *b){
    ...
    if(main==31&&sub==310){
        if(native_cg_open(b))return error(b,"31/310 CG resources invalid (arguments preserved)");
        v->sp--;b->handled++;return kvm_resume(v);
    }
    if(main==31&&sub==111){
