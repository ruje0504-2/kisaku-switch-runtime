...
int kmov_open(KMov *m,const uint8_t *d,size_t n,unsigned entry){
    memset(m,0,sizeof(*m));
    if(!d||n<534||u32(d)!=1||!memchr(d+4,0,261)||!memchr(d+265,0,261))return fail(m,"MOV header invalid");
    unsigned count=u32(d+526);
    if(!count||count>256||entry>=count||(size_t)count*4>n-530)return fail(m,"MOV entry count invalid");
    for(unsigned i=0;i<count;i++){size_t ip=u32(d+530+i*4);if(ip<530+count*4||ip>n-12)return fail(m,"MOV entry offset invalid");}
    m->data=d;m->size=n;m->count=count;m->requested=entry;m->video=(const char *)d+4;m->audio=(const char *)d+265;select_entry(m);return 0;
}
...
int kmov_next(KMov *m){
    ...
    for(unsigned guard=0;guard<10000;guard++){
        if(m->ip>m->size||m->size-m->ip<12)return fail(m,"MOV command truncated");
        const uint8_t *p=m->data+m->ip;unsigned op=u32(p);int32_t a=(int32_t)u32(p+4),b=(int32_t)u32(p+8);m->ip+=12;
        switch(op){
