...
typedef struct {
    ...
    unsigned present[10],fallen[10],active,gutter,loud_hit;
    unsigned contacts,ball_contacts;
} KBowlingWorld;
...
static inline int kbowling_world_tick(KBowlingWorld *w,uint32_t elapsed){
    ...
    for(unsigned i=0;i<10;i++){previous[i]=kbowling_pin_position(&w->pins[i]);w->pins[i].elapsed+=10;}
    unsigned new_hits=0;
    for(unsigned i=0;i<10;i++)if(w->present[i]){
        for(unsigned j=i+1;j<10;j++)if(w->present[j]){
            ...
            if(hit){if(!w->fallen[i]||!w->fallen[j])new_hits+=((hit&1)!=0)+((hit&2)!=0);w->fallen[i]=w->fallen[j]=1;w->contacts++;}
        }
    ...
    for(unsigned i=0;i<10;i++)if(w->present[i]&&kbowling_ball_contact(&w->pins[i],w->pin_width,w->ball,direction,w->ball_width,w->ball_speed)){
        if(!w->fallen[i])new_hits++;
        w->fallen[i]=1;w->ball_contacts++;
    }
