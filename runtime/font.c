...
KFont *kfont_open(const char *path,int simplified){
    ...
    if(path&&*path){
        ...
        for(unsigned i=0;i<3;i++){
            PlFontData data;
            if(R_SUCCEEDED(plGetSharedFontByType(&data,types[i]))&&!FT_New_Memory_Face(f->library,data.address,data.size,0,&f->faces[f->count]))f->count++;
        }
#elif defined(__APPLE__)
        (void)simplified;
        if(!FT_New_Face(f->library,"/System/Library/Fonts/Hiragino Sans GB.ttc",0,&f->faces[0]))f->count=1;
#else
        (void)simplified;
#endif
    }
...
int kfont_draw(KFont *f,KImage *dst,uint32_t cp,int x,int y,unsigned width,unsigned height,uint32_t rgb){
    ...
    for(unsigned i=0;i<f->count;i++)if((index=FT_Get_Char_Index(f->faces[i],cp))){face=f->faces[i];break;}
