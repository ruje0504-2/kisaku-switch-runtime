...
## 2026-09-23 呈现性能与第三核分工

- 保持960×720高清文字、外描边、CAS及60 FPS目标节拍。原始游戏表面和脚本时钟不变，不靠降帧/降画质减少负载。
- 新增present_worker：Switch固定core 2、优先级0x2c、1 MiB栈，按需准备当前帧的高清文字/选项合成；主线程同期只执行音频混音和SDL清屏，在读取结果前通过条件变量等待。无下一帧队列；输入、VM推进、设置变更、存读档替换和退出释放均在任务结束之后。原语音解码仍core 1，图片解码仍core 2但优先级改0x2d，低于当前帧准备的0x2c；空闲线程休眠，不人为占满核心。
- 高清底图仅合成一次，再叠加低清参考文字，不再重复绘制窗口、按钮和底色；原版分别/255取整保持。遮挡核对先比较整行，相同行跳过逐像素扫描。
- GLES自有纹理直接上传BGRA字节，通过片元着色器.bgr解释通道，去掉CPU整屏通道交换。整帧内容不变时复用上传；仍完整恢复SDL的GL状态。100帧稳定底图专项仅1次上传，真实GLES颜色/文字/菜单像素对比通过。
- 高清文字纹理保存独立快照，仅上传首尾变化行之间的范围；文字擦除同样会触发上传。30帧稳定文字仅1次上传，单行变化仅上传960×1，首尾不同行变化及原有GLES合成检查通过。原生CG面板复用纹理，12帧仅1次上传；CG与历史的被遮住剧情底图不再合成/锐化。
- 对照bdc9a53的bootstrap.c重新构建基线，使用同一原始开场检查点：300次稳定高清准备的主机CPU时间108.672ms→62.990ms（约减少42%）。raw/clean/overlay以及额外遮挡后的clean/overlay哈希完全相同。日志local/optimize-benchmark-before.log、local/optimize-benchmark-after.log；tests/present_benchmark.c由build-host.sh构建。此为主机微基准，不是Switch帧率或总体CPU降幅。
- 并发专项：真实开场逐字显示、音频混音重叠、60次选项准备与退出时等待未结束任务，串行/工作线程结果逐像素相同，原始图层不变；ThreadSanitizer无报告（local/optimize-tsan.log）。SDL/GLES真实六面板与高清文字专项通过（local/optimize-gles.log）。
- Switch实机CPU核心占用、耗电、帧率和长时间稳定性尚未验证。核心2与图像解码竞争时的实际收益没有实机测量，不能据主机微基准推断。
...
## 2026-09-27：31/1011 action 11 与 31/1010 的"流播放中"语义纠正（可达性扫描产物）

- 来源：682 模块可达性扫描（`deepseek/analysis/reachability-sweep.md`）：起初 108 个模块报错，
  其中 **70 个**卡在 `31/1011 action 11`、**6 个**卡在 `31/1010`，两者都是移植版把原版的
  **静默跳过**写成了报错。
- `31/1011` action 11（原生 `4f9a3e`）：先算 `486130(4e2aa0()->[0xc0])` —— 取流表当前条目
  （索引 `+0x0c`）并读其 `+0xbc`（`486c40`）；非零时 `jne 4f9a63` **跳过** `5042d0`，不写 FLAG、
  不报错；为零才执行成员/登记数匹配。`aoi_h1.mes` 等 70 个模块是"1010 选流 → 1011/11 登记"成对调用，
  前 9 对通过、第 10 对流已活跃 → 命中该守卫。
- `31/1010`（原生 `4f9b50`）：弹 1 个操作数 → `486160(流表, action)`；无当前条目立即返回，
  有则 `486cd0` → `421310` 执行 `entry->[0x24]->[0x1e8] = action`（写命令字段），**无失败路径**。
  移植版原先只在"无流"时 no-op、有流时报 `not yet mapped`。
- 实现：`bootstrap.c` 中 1011/11 在 `b->video||b->mov_data` 时**消费两个操作数后返回**（不登记），
  1010 去掉 `!video&&!mov_data` 前置条件、两种状态都消费操作数；两处注释写明原版地址与语义。
- 测试：`kisaku_bootstrap_test.c` 原有"有视频时 1011/11 报错"的断言按原生语义改为
  "静默跳过且不写 FLAG 字节"（`video` 与 `mov_data` 各验证一次）。
- 验证：可达性扫描 **574/108 → 645/37**（剩余 37 全为"模块被当作独立脚本从第 0 条指令进入、
  缺少真实调用者上下文"的入场偏差：小游戏参数向量 7、CG 画布前置状态 11、31/520 轨道 5、
  日记记录 3、bowl2nd 1、`sp=0` 缺操作数 9…）；完整 `test-host.sh` **102 条 PASS**；
  自然全新存档探针 3417/2441/212 不变；Switch `build-switch.sh` 通过。
